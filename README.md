...
Powered by the **Google Gemini API**, this platform is lightning fast, highly scalable, and designed to run on lightweight cloud infrastructure (like Railway) using a multi-tenant vector database.
...
│                                                                             │
│  ┌─────────────┐  ┌──────────────┐  ┌───────────┐                           │
│  │  Frontend    │  │   Backend    │  │  Worker   │     [ Internet ]         │
│  │  Next.js 15  │──│  FastAPI     │──│  Celery   │────▶ Gemini API          │
...
Upload PDF ──▶ MinIO Storage ──▶ Celery Worker ──▶ Extract (unstructured)
                               ...
                               Chunk (hierarchical)
                                       │
                                       ▼
                          Embed (Google Gemini API)
                                       │
                                       ▼
                              Store in Qdrant 
                                       │
                                       ▼
Query ──▶ Route ──▶ Vector Search ──▶ Rerank ──▶ LLM (Google Gemini) ──▶ Answer
```
